## Variables, commands/statements/expressions, operators ##
...
&&And
||Or
XORExclusive orTrue only if one operand is true:
A XOR BB=trueB=falseA=truefalsetrueA=falsetruefalse
EQVEquivalenceTrue if both operands are the same:
A EQV BB=trueB=falseA=truetruefalseA=falsefalsetrue
IMPImplicationOnly if the first operand is true, the second operand must also be true. Otherwise true.

A IMP BB=trueB=falseA=truetruefalseA=falsetruetrue
Equivalent to !A || B

Basically the expression is true, unless the first operand is true and the second is false.

And example might be in a situation where a function can optionally filter by date, eg:

getRecords(boolean filterByDate=false, date date)

Argument validation could be:

argsAreValid = filterByDate IMP structKeyExists(arguments, "date"));
if (!argsAreValid){
    throw(type="InvalidArgumentsException");
}

This operator has a fundamental flaw in that it's not "short circuited": the second operand is always evaluated even if the first makes the condition true (eg: the first operand is a false condition). This prevents the most useful potential use of this operator which would be:
...
This fails because the second operand is evaluated even if URL.someValue doesn't exist, which would have caused the first operand to cause a false condition, and the entire expression is therefore false even before considering the second operand.

I also think it's a difficult operator to understand, so simply using !A || B is easier to follow than A IMP B.
...
containsChecks whether a string contains a substringa CONTAINS b is true if the string a contains substring b.
EG: "slaughter" CONTAINS "laughter" is true. Inappropriate, but true.

I'd use the find() function instead, TBHdoes not containThe opposite of the above



Conditional assignment

?:Boolean assignmentresult = booleanCondition ? valueIfTrue : valueIfFalse

EG:

whatToDoToday = isItTheWeekend ? "go to the beach" : "go to work"?:Null coalescingresult = valueIfNotNull ?: valueIfFirstOperandIsNull

EG:

a = 1;
b = a ?: 2; // a is not null, so b=1
...
c = d ?: 3; // d is not defined which is interpreted as "null" in CFML, so c=3
